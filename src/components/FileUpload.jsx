...
const FileUpload = ({ 
  ...
  const getFileIcon = (fileType) => {
    if (fileType.startsWith('image/')) return <Image className="text-blue-500 h-5 w-5" />;
    if (fileType.startsWith('video/')) return <Video className="text-purple-500 h-5 w-5" />;
    if (fileType === 'application/pdf') return <PdfIcon className="text-red-500 h-5 w-5" />;
    if (fileType.includes('document') || fileType.includes('text')) return <DocumentIcon className="text-green-500 h-5 w-5" />;
            ...
            {files.map((fileItem) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-[#40444D] truncate" title={fileItem.name}>
                    {fileItem.name}
                  </p>
