...
export default function NewEventDialogRHF({ open = false, onClose = () => { }, onSubmit = () => { } }) {
  const {
    control,
    register,
    handleSubmit,
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4 overflow-y-auto">
      ...
      >
        <h2 className="text-2xl text-[#40444D] text-center font-bold font-sans">Create New Event</h2>
            ...
            <Controller
              ...
              render={({ field }) => (
                <Input
                  type="datetime-local"
                  {...field}
                  className={`border ${errors.start_time ? "border-red-500" : ""}`}
                />
              )}
            />
            ...
            <Controller
              ...
              render={({ field }) => (
                <Input
                  type="datetime-local"
                  {...field}
                  className={`border ${errors.end_time ? "border-red-500" : ""}`}
                />
              )}
            />
            ...
            <Controller
              ...
              render={({ field }) => (
                <Input
                  type="text"
                  {...field}
                  className={`border ${errors.category_id ? "border-red-500" : ""}`}
                />
              )}
            />
        ...
        <div className="pt-4">
          <div className="flex justify-between items-center mb-2">
            ...
            <div
              onClick={() => appendReminder({ type: "sound", timing: "minutes_before", value: 15, relative_to: "start" })}
          ...
          {reminderFields.map((field, index) => (
            <div key={field.id} className="flex items-center justify-between">
              ...
              <div className="flex items-center justify-end w-20 ">
                <Trash2 onClick={() => removeReminder(index)} className="text-[#6366F1] hover:text-red-500 transition-all duration-300 ease-in-out h-5 w-5 cursor-pointer" />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="pt-4">
          <div className="flex justify-between items-center mb-2">
            <h3 className="text-xl font-semibold text-[#40444D]">Participants</h3>
            <div
              onClick={() => appendParticipant({ email: "", role: "", status: "pending", comment: "" })}
              className="p-1 rounded-full cursor-pointer hover:bg-[#6366F1] hover:text-white transition-all duration-300 ease-in-out"
            >
              <Plus className="h-5 w-5" />
            </div>
          ...
          {participantFields.map((field, index) => (
            <div key={field.id} className="flex items-center justify-between">
              <div className="flex items-center justify-between gap-2">
                ...
                <Input {...register(`participants.${index}.comment`)} placeholder="Comment" />

              </div>
              {/* Remove */}
              <div className="flex items-center justify-end w-20 ">
                <Trash2 onClick={() => removeParticipant(index)} className="text-[#6366F1] hover:text-red-500 transition-all duration-300 ease-in-out h-5 w-5 cursor-pointer" />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex justify-end gap-4">
          <Button onClick={onClose} className="bg-gray-300 text-black hover:bg-gray-400 cursor-pointer">Cancel</Button>
          <Button type="submit" className="bg-[#6366F1] text-white hover:bg-[#4e5564] cursor-pointer">Submit</Button>
        </div>
      </form>
    </div>
  );
