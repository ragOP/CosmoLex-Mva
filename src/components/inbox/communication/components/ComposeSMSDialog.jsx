...
const ComposeSMSDialog = ({ open, onClose, onSuccess, matterId }) => {
  ...
  const { data: smsSearchResults, isLoading: contactsLoading } = useQuery({
    ...
    queryFn: () => searchUsers({ searchBar: debouncedSearchTerm, role_type: '' }, 2), // Type 2 for SMS
    enabled: open && debouncedSearchTerm.length > 0
  });

  const fromPhones = metaData?.from || [];
  const metaContacts = Array.isArray(metaData?.to) ? metaData.to : (metaData?.to ? [metaData.to] : []);
  const searchedContacts = smsSearchResults?.data || [];
  
  // Show meta data contacts if no search, otherwise show search results
  const availableContacts = debouncedSearchTerm.length > 0 ? searchedContacts : metaContacts;
  ...
  const handleInputChange = (field, value) => {
    ...
    if (field === 'recipient') {
      console.log('Recipient value type:', typeof value, 'Array?', Array.isArray(value));
  ...
  const handleFromPhoneSelect = (phone) => {
    ...
    const phoneNumber = typeof phone === 'object' && phone !== null ? phone.number : phone;
    ...
    const formatRecipients = (recipients) => {
      ...
      const formatted = recipients.map(recipient => {
        ...
        if (typeof recipient === 'object' && recipient !== null) {
          ...
          const phone = recipient.recipient || recipient.phone || recipient.phoneNumber || recipient.number;
          console.log('Extracted phone from recipient object:', phone, 'from:', recipient);
          return phone;
        }
        return recipient;
            ...
            <Stack direction="row" alignItems="center" spacing={2}>
              ...
              <Box sx={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                  {formData.from ? (
                    <Chip
                      label={typeof formData.from === 'object' && formData.from !== null ? formData.from.number : formData.from}
                  ...
                  {formData.recipient.length > 0 ? (
                    formData.recipient.map((recipient, index) => {
                      ...
                      if (typeof recipient === 'string') {
                        ...
                        displayLabel = recipient.recipient || recipient.phone || recipient.phoneNumber || recipient.number || JSON.stringify(recipient);
                        // If recipient.recipient is an object, try to extract phone number
                        if (typeof displayLabel === 'object' && displayLabel !== null) {
                          displayLabel = displayLabel.number || displayLabel.phone || JSON.stringify(displayLabel);
                        }
                      }
                        ...
                        <ListItemButton
                          onClick={() => {
                            ...
                            if (isSelected) {
                              ...
                              console.log('Removing recipient, new list:', newRecipients);
                              handleInputChange('recipient', newRecipients);
                            } else {
                              const newRecipients = [...formData.recipient, contact];
                              console.log('Adding recipient, new list:', newRecipients);
                              handleInputChange('recipient', newRecipients);
                            }
                          }}
                          sx={{
                          ...
                          <ListItemText
                            ...
                            secondary={
                              <Typography variant="caption" color="text.secondary">
                                {typeof contact.recipient === 'object' && contact.recipient !== null 
                                  ? contact.recipient.number || contact.recipient.phone || JSON.stringify(contact.recipient)
      ...
      >
        <DialogTitle sx={{ pb: 1 }}>
          <Stack direction="row" justifyContent="space-between" alignItems="center">
            ...
            <IconButton onClick={() => setShowFromPhoneDialog(false)} size="small">
              <X size={20} />
            </IconButton>
          </Stack>
        </DialogTitle>
